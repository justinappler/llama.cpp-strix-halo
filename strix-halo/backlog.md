...
| 2 | ~~**MMQ port-off A/B**~~ **DONE 2026-08-29** - three patches together **+26.6% to +28.8%**; the **MMQ table alone only +2.1% to +4.2%**. Remaining: isolate the FA TILE case (#8), which is now the prime suspect for the bulk of the gain, and the `occupancy=4` sweep. | S | attribution (surprising) + S | The table is 8-15% of the fork's prefill gain, not the whole of it. Changes the upstreaming pitch: a table-only PR is a +2-4% change, in line with what #26284 is landing for RDNA3.0, not a headline win. |

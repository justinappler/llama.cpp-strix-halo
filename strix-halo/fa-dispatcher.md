...
**Status: attempted, abandoned. JohannesGaessler is doing the proper fix upstream.** The naive port (widen guards, add RDNA3 `make_identity_mat`, add 4-arg RDNA3 WMMA intrinsic) compiles and runs — but produces incorrect output because RDNA3's WMMA f16→f16 accumulator is unpacked (1 half per 32-bit lane) while RDNA4's is packed (2 halves per lane) and the surrounding tile/FA math assumes packed. Upstream attempt [PR #19063](https://github.com/ggml-org/llama.cpp/pull/19063) hit exactly this wall and was closed by its own author. **As of 2025-10-29, JohannesGaessler is adding AMD-WMMA + AMD-MFMA + Volta support to `mma.cuh` and porting the FA-MMA kernel onto them, then removing the WMMA kernel entirely** ([PR #16827 comment](https://github.com/ggml-org/llama.cpp/pull/16827#issuecomment-3454830174)). PR #22051 (mma data loading refactor, merged 2026-04-17) and PR #22298 (MMQ stream-k overhead, merged 2026-04-26) are early steps in that chain. The FA-MMA-on-AMD-WMMA enablement PR hasn't landed yet — when it does, this dead end resolves itself. See [Upstreamability](#upstreamability).
...
### Update 2026-04-29: superseded by JohannesGaessler's roadmap

The "multi-week investigation" framing above is stale. JohannesGaessler [committed publicly](https://github.com/ggml-org/llama.cpp/pull/16827#issuecomment-3454830174) on 2025-10-29 to adding AMD-WMMA + AMD-MFMA + Volta support to `mma.cuh` and porting the FA-MMA kernel onto them — then deleting the WMMA kernel. That's exactly the work this section was scoping out. AMD shipped him a Strix Halo box specifically to enable this. His ETA was ~1 month from Oct 2025; we're 6 months out, with PR #22051 (merged 2026-04-17) and PR #22298 (merged 2026-04-26) being the early steps.

Pursuing Option A/B/C ourselves now would (a) duplicate his work, (b) almost certainly produce worse code than someone with maintainer-level familiarity with `mma.cuh`, and (c) be discarded on rebase the moment his FA-MMA-on-AMD-WMMA PR merges. **The right play is to track his progress.** The fork's action item shrinks from "multi-week kernel investigation" to "watch for the next PR in the chain, drop our `a6a27b66` rocWMMA tuning when `fattn-wmma-f16.cuh` is removed."

If JohannesGaessler's roadmap stalls past mid-2026 or pivots to skip RDNA3/3.5, the multi-week investigation becomes live again — but we'd reassess at that point, not now.

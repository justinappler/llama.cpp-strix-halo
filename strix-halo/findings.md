...
| 9 | [RDNA3.5 MMQ config table](mmq-rdna3_5-config-table.md) | **+2.1% to +4.2% prefill on its own** (measured 2026-08-29, table-only build vs clean upstream). The *three patches together* are worth +26.6% to +28.8%. | **Live on master, and now decomposed.** Two controls were run against the same base `c841aeeb8`: all three patches (+26.6-28.8%) and this table alone (+2.1-4.2%). **The table accounts for only 8-15% of the fork's prefill gain** - the long-carried "+27% from the MMQ retune" claim was wrong, and was never actually attributed until now. Decode flat to within 0.2% in both. The remainder is the `J_max` cap (measured flat, #10) and the FA TILE case (#8), so #8 is the likely dominant contributor and is not yet isolated. |
...
| 11 | [FA MMA_F16 at D=256 via upstream PR #26419](fa-mma-d256-26419.md) | **Regression: -4.7% pp @ d=16k on stock upstream, -11.6% on our tree**; decode flat | **Evaluated and declined 2026-08-29.** Four-arm matrix on one host. The PR routes AMD WMMA to MMA at head dim 256 and gfx1151 is inside its gate, so it would have retired our `fattn-tile.cuh` patch silently, by dispatch, with no merge conflict. It loses on both bases and the loss *grows* with depth - the opposite trend from the RDNA4 reports on the thread. Nothing changed on master; #8's TILE patch is vindicated. Numbers are worth reporting on the PR, which has no RDNA3.5 data point. |

## Two caveats worth carrying forward

**~~Finding #9 has never had its control run.~~ Resolved 2026-08-29, and it overturned the headline claim.** Two controls against the same base `c841aeeb8`:

- **All three patches vs clean upstream: +26.6% to +28.8% prefill**, decode flat within 0.2%.
- **The MMQ table alone vs clean upstream: +2.1% to +4.2%.**

So the fork is worth about what was always claimed, but **the MMQ retune is not where it comes from** - the table is only 8-15% of the gain. Every prior statement attributing "+27%" to the MMQ port was a bundle delta wearing one patch's name. `J_max=48` measured flat back in #10, which leaves the FA TILE case (#8) as the likely dominant contributor; it has not been isolated yet.

Two lessons. The control was one extra build and only got run because an unrelated experiment needed clean upstream as a baseline anyway - four months of hedging cost more than the build did. And a bundle control is not an attribution: measuring three patches together and labelling the result with one patch's name is the same error in a new coat.

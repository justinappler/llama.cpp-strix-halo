...
| 4 | [ROCm config flags: unroll-threshold + `HIPBLASLT_BATCHED=0`](rocm-config.md) | Community reported 2x pp on other models; **null** on Qwen 3.6 | **Bench null, kept on** as AMD-recommended safety nets. Flags live in the deploy config, not this repo. |
...
| 9 | [RDNA3.5 MMQ config table](mmq-rdna3_5-config-table.md) | Bundle delta **+5.8% pp @ d=0, +5.9% @ d=16k**; the port's own share is **unmeasured** | **Live on master**, reshaped 2026-08-02 onto upstream's own rdna3-5 table (see below). Correctness clean on real gfx1151. The port-off A/B was never run, so the gain is not attributed. |
...
**Finding #9's headline number is a bundle delta, not an A/B.** The build it was measured against differs by 234 upstream commits, a ROCm 7.13 -> 7.14.0 bump, *and* the port. tg128 rose ~5% in the same run, and MMQ tuning cannot move decode - that is the tell that much of the gain belongs to upstream and ROCm. The port is very likely still winning, but it has not been re-proven since 2026-04. See [mmq-rdna3_5-config-table.md § Outcome](mmq-rdna3_5-config-table.md#outcome).

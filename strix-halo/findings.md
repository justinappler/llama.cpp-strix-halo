...
| 9 | [RDNA3.5 MMQ config table](mmq-rdna3_5-config-table.md) | The port's own share is still **unmeasured**; what is known is that losing it would cost 27-37% | **Live on master**, reshaped 2026-08-02 onto upstream's own rdna3-5 table (see below). Correctness clean on real gfx1151. Re-benched after the reshape: prefill flat, so the tuning carried across intact. The port-off A/B still has not been run. |
